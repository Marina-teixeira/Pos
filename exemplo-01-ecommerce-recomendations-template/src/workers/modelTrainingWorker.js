...
function makeContext(catalog, users) {
    ...
    const colors = [...new Set(catalog.map(p => p.color))] // Para garantir que não venham duplicadas
...
function encodeProduct(product, conetext) {
    ...
    const color = oneHotWeighted(
        context.colorIndexIndex[product.color],
        context.numCategories,
        WEIGHTS.category
    )
...
async function trainModel({ users }) {
    ...
    context.productVectors = catalog.map(product => {
        return {
            name: product.name,
            meta: {...product},
            vector: encodeProduct(product,).dataSync()
        }
    })
